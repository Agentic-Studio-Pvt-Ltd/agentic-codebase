...
**This file is the spec of record.** Read it before proposing structure, and keep it in sync when
decisions change. The product direction it encodes — how much a run builds, and what it asks the
user — is deliberate; read the invariants below before changing either.

The deliverable is `skills/agentic-codebase/`: a `SKILL.md` orchestrator, four stdlib-only Python analyzers
...
library — no `pip install`, no `package.json`, no `node_modules`. A global `CLAUDE.md` that sets
JavaScript-toolchain defaults does not apply here; there is nothing to run them against.

The analyzers' own selftests are developer tooling and must never be
invoked during a real run:

```bash
python3 skills/agentic-codebase/scripts/discover.py         --selftest
python3 skills/agentic-codebase/scripts/mine_git.py         --selftest
python3 skills/agentic-codebase/scripts/mine_transcripts.py --selftest
python3 skills/agentic-codebase/scripts/verify_artifacts.py --selftest
```
...
```bash
A=skills/agentic-codebase/scripts
...
2. **Core workflow** (target-agnostic): diagnose → map findings to artifacts → walk the
...
The parts most easily broken by a plausible-looking change:

- **Additive and reversible.** Never overwrite an existing file. The index doc is merged by appending
  inside `agentic-codebase:begin`/`agentic-codebase:end` markers. Changes land on branch `agentic-setup/<yyyy-mm-dd>`
...
- **The undo is generated from `build-manifest.json`, never from a template line.** The recurring
  defect class in this repo's history is *agentic-codebase writes a file one way and un-writes it another*:
...
- **Idempotent.** Every generated file carries an `agentic-codebase-id`; a rerun updates in place.
...
- **"Covered" has one definition** (`blueprint.md` §2.1): an artifact of the **same type, inside this
  ...
  2026-09-07 reference run lost some twenty artifacts to the opposite readings of each of these.
...
- **agentic-codebase never pushes and never opens a pull request.** The branch is left for the user.
...
- ~~The name~~ — **settled 2026-09-17: `agentic-codebase`, under `Agentic-Studio-Pvt-Ltd`.**
  The working title `agentify` was dropped: npm holds `agentify` (v0.0.1, `unadlib`, 2024-06-09)
  and `agentify-sh` is an active GitHub org in this exact niche. The rename landed before first
  publish, which is the only moment it is free — the marker pair and the `agentic-codebase-id`
  key are written into *user* repos, so a later rename orphans every setup already built.
- ~~The Agentic Studio upsell URL~~ — **settled 2026-09-15: `https://theagentic.studio`.** All
  ...
  is the only form, so no fill pass can ship a placeholder. Each attribution surface carries its own
  literal `utm_medium` (`run-summary`, `report`, `index-doc`) so bookings trace to the surface.
...
- Every default here answers a question that was weighed once. Changing one is fair game; doing
  it without saying which invariant it trades against is not.
