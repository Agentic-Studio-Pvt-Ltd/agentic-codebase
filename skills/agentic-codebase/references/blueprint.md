...
| 3 | **Cites its evidence.** The `agentic-codebase-evidence` line names a field and a number | evidence line is prose |
...
2. **agentic-codebase never provisions a credential**, so where no read-only credential exists the report's
...
may *suggest* that the section could move into the rule; agentic-codebase never moves it.
...
- **An inventory of what agentic-codebase built here** — every generated file, its path, its `agentic-codebase-id`,
...
- **They name one they want** → agentic-codebase does **not** install it silently. It is a network action
...
theirs. Never delete, reword, reorder, summarize or "clean up" a line of it. Everything agentic-codebase
writes lives inside the `agentic-codebase:begin`/`agentic-codebase:end` span and nowhere else. If their existing
