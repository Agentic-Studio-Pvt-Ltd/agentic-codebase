...
## 1. Mapping table (expanded with thresholds)
...
- **One finding may produce more than one artifact.** The canonical example is exact: a repeated
...
agentic-codebase builds whose normal operation is to **refuse** something. Everything else adds: a prose rule
adds a sentence the model may follow, a hook usually warns, a skill offers steps. A command policy —
`decision = "forbidden"` in `.codex/rules/agentic-codebase.rules` on Codex, a blocking `PreToolUse` hook on
...
| Permissions | `settings-permissions.json.tmpl` | a merge into `.claude/settings.json` (`permissions.allow` / `.deny`) | `codex-rules.rules.tmpl` — the Starlark policy **is** this target's permission surface | `.codex/rules/agentic-codebase.rules`, the same file the command-policy row writes |
...
file — `.claude/settings.json`, `.codex/hooks.json` — carries no agentic-codebase metadata at all, because
JSON has no comments and Codex's hooks file rejects unknown top-level keys outright (measured: an
`_agentic-codebase` key there loaded **zero** hooks, with no error the user ever sees). For both, the
...
they have no template — the adapter carries the body, the required `AGENTIC_CODEBASE_DRYRUN` escape hatch
...
```
<!--
agentic-codebase-id: <kebab-name>
agentic-codebase-version: 1
agentic-codebase-generated: <yyyy-mm-dd>
agentic-codebase-evidence: <the row 9 evidence string>
...
The rows added for Codex slot into that same order and add no step: `.codex/rules/agentic-codebase.rules`
...
its permissions step writes the same `.codex/rules/agentic-codebase.rules` the command-policy rows write, so
...
- The same string goes into three places: the candidate list (phase 4), the plan entry (phase 6),
  and the generated file's `agentic-codebase-evidence:` frontmatter line (phase 7). They must match.
...
`CLAUDE.md`, low-confidence candidates are **listed and not built** unless the user opts in — that is
...
tests explicitly, so they survive the step test rather than being steps you assumed. Per `CLAUDE.md`, the
...
in phase 7 that same string becomes the `agentic-codebase-evidence:` frontmatter value of the generated
...
audit-only mode (since retired — this is a record of a past run, not an instruction). This
...
### 7.6 Worked negative — `commit-acme`, the right count on the wrong artifact
...
string that gets copied verbatim into `plan.md`, `report.md` and an `agentic-codebase-evidence:` line — the
...
as a skill at `.claude/skills/commit-acme/SKILL.md`, score 13.
...
### 7.7 Worked edge — `agentic-codebase-conventions`, a count sitting exactly on the line
