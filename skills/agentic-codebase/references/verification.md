...
2. **Fixtures live outside the repo.** Create them under a temp directory (e.g.
   `$(mktemp -d)/agentic-codebase-verify`). Never create a fixture file inside `src/`, `tests/`, or any
...
8. **Never execute a hook without asking first, and never execute one agentic-codebase did not write.**
...
### 2.0 Before you run it: agentic-codebase's own output belongs in the manifest
...
that carried no `agentic-codebase-id` anywhere in it and no check noticed, because the file describing
...
1. **The manifest carries its own identity.** Add one inert top-level key, the same shape
   ...
   ```json
   "_agentic-codebase": {
     "agentic-codebase-id": "build-manifest",
     "agentic-codebase-version": 1,
     "agentic-codebase-generated": "2026-09-04",
     "agentic-codebase-evidence": "index of the 12 artifacts this run built",
...
2. **Add two entries now**, using the types `plan` and `manifest`:
   ...
   Keep `action: created` for all three, **including on a rerun**. agentic-codebase owns these three files
   ...
   `mcp` and `plugin` are, and its `metadata` check reads the `_agentic-codebase` object above. The ids
...
file existing, being non-empty, parsing as JSON, and carrying `_agentic-codebase`. None of them depends on
...
you happen to be standing in the repo root, and agentic-codebase is often invoked from somewhere else;
...
it, `rule_contradiction` compares rules only with each other, and the contract's other half — "check
...
| `id_unique` | no two manifest entries share an `agentic-codebase-id` | fail |
...
| `metadata` | `agentic-codebase-id` / `-version` / `-generated` / `-evidence` present, the id matches the manifest, and the file says `Safe to delete or edit.` | fail (warn for JSON and for `modified` files) |
| `evidence_match` | the file's `agentic-codebase-evidence` line is the **same string** as its manifest entry's `evidence`. Quoting, spacing, smart punctuation and case do not count as a difference; a truncation (one side a prefix of the other, both ≥ 20 chars) is a warn; anything else is drift and a fail. Not reported when either side is absent — `manifest_entry` and `metadata` already own those | fail |
...
- **Read the `rule_contradiction` PASS row, do not skim it.** Its detail says whether conventions
  ...
  both halves ran; `…conventions NOT checked` means `--discovery` was missing and half of `CLAUDE.md`
...
whole tool where code agentic-codebase wrote gets executed on the user's machine, and therefore the only
remaining route to the network in a tool that promises "local only, no network calls".
...
**agentic-codebase never executes a hook it did not write.** Even with `--exec-hooks`, a hook runs only
...
environment cut down to `PATH`, `HOME`, `TMPDIR`, `AGENTIC_CODEBASE_VERIFY` and **the variables its own
...
| A **non-zero exit at all** | 2 blocks; other non-zero codes are a non-blocking error | **a fail.** Only 0 and 2 are specified, agentic-codebase's generated Codex scripts exit 0 always and express every decision in JSON (`adapters/codex.md` §4.3) |
...
outright that `Bash` never fires on Codex, and agentic-codebase shipped shell matchers that excluded the one
...
- `hook_static_scan` found a network call → the hook is wrong, not the check. agentic-codebase never
...
| trusted, file well-formed | one entry per generated hook | `[]` | **pass** on `eventName`, `matcher`, `sourcePath`, and `source: "project"` — `"user"` would mean something wrote to the global file, which agentic-codebase must never do. `trustStatus` is `"untrusted"` here too; that is the *armed* question, not the *loaded* one. |
...
. Measured: one `initialize` against a scratch home left seven `plugins-clone-*` working
...
security decision belonging to the person whose machine it is; agentic-codebase's job is to tell them the
...
2. **A repo-scoped artifact is inert until the project is trusted.** With no trusted
   ...
   `.codex/rules/` and `<repo>/.codex/config.toml` all do nothing, silently. agentic-codebase never edits
...
3. **agentic-codebase cannot restart the user's session**, so anything needing a fresh one is a manual item.
...
by hand as well — `codex execpolicy check --pretty --rules .codex/rules/agentic-codebase.rules -- <a
...
Also check that each rule still carries its evidence line and its `agentic-codebase-id`, and that no rule
...
the rule's `agentic-codebase-evidence` is no longer the string the manifest carries, which means the file
...
**Index doc section.** Pass: the appended section is delimited by its `agentic-codebase-id` markers, the
...
3. **No `allow` or `ask` entry matches a secret path.** Grep the allow and ask arrays for `.env`,
   ...
   agentic-codebase pre-approving a read it is forbidden to make. Remediation is deletion, not narrowing.
...
On Codex there is no permissions block: the surface is `.codex/rules/agentic-codebase.rules`, already
covered by §6.2's command-policy checks, and `codex execpolicy check` is the whole test.

**Plugin manifest — not built.** agentic-codebase emits no plugin manifest on either target
(`adapters/capabilities.md`). The `plugin_paths` static check (§2) remains, because a rerun over a
repo an older agentic-codebase wrote into can still find one in the manifest and must not report a dangling
...
6. **Now add the report's `artifacts[]` entry and verify it.** `report.md` is the last thing
   agentic-codebase writes and the one file step 1 could not cover, because it did not exist yet. Append:
   ...
   **The path is already in the undo lists — 4a put it there. Do not add it again**, and above all
   ...
   - **not clean** → fix `report.md` (a missing `agentic-codebase-id`, an empty file, a path that escaped
   ...
   the `_agentic-codebase` identity, none of which depends on its contents. See §2.0.
...
`CLAUDE.md` promises `reversible` and `CLAUDE.md` promises an uninstall path in every report. **A §10 that
...
when git cannot, the file's **format** says which script takes agentic-codebase's contribution out. There
are two scripts in `report-template.md` §3.1 and they are not interchangeable:

| the file agentic-codebase changed | mechanism | keyed on |
|---|---|---|
| a **marked text file** — `CLAUDE.md`, a rule, a shell config | the marker script | the `agentic-codebase:begin` / `agentic-codebase:end` pair for its `agentic-codebase-id` |
| a **structurally merged JSON config** — `.claude/settings.json` or `.codex/hooks.json`, `.mcp.json` (`type: settings` / `type: mcp` **and a `.json` file**) | the **JSON un-merge script** | the entries this run merged in: each hook artifact's manifest `command`, each `mcpServers` name, and an `_agentic-codebase` object whose `agentic-codebase-id` matches |
...
any other file of its shape: `<plan-dir>/codex-mcp.toml` is a draft agentic-codebase created whole and
...
and there is no TOML writer in the standard library at any version, which is why agentic-codebase never
...
the marker script finds no block, prints `SKIP <path>: no agentic-codebase block id=…`, **exits 0**, and
...
agentic-codebase block id=enforce-bun`, exit 0, file byte-identical before and after, `enforce-bun` still
...
- an `action: modified` **marked text file** → the marker script's `BLOCKS` list names it, with its
  `agentic-codebase-id` and `pre_existing_sha256`;
...
- **fail, a JSON config is in an `rm -f` operand list** — a blind `rm` destroys whatever the user
  ...
  agentic-codebase created it and nothing else is left, and keeps it when something is.
...
- **base tip moved** — a base branch whose tip no longer equals `manifest.base_commit` means the run
  committed to it. That is a `CLAUDE.md` violation, not a reporting problem. Stop and tell the user.
...
- `action: modified`, **a marked text file** — the file exists, holds exactly one
  `agentic-codebase:begin`/`agentic-codebase:end` pair for its `id` (the static pass's `markers` row already told
...
- `action: modified`, **a JSON config** (`type: settings` or `type: mcp` in a `.json` file) — it has **no markers and
  ...
  an `_agentic-codebase` object whose `agentic-codebase-id` matches. Neither hook registry carries an `_agentic-codebase`:
  `templates/settings-hooks.json.tmpl` step 6 forbids merging that key into `settings.json`, and
  `.codex/hooks.json` accepts **only** `description` and `hooks` — an `_agentic-codebase` beside them makes
  Codex reject the file and load zero hooks. So on both targets the command path is the whole of
  agentic-codebase's mark on that file. Reading `restore: span` here as "the marker script handles it" is
...
1. **Drop the `FILE` line** of every artifact with `action: created` — **including the stand-in
   ...
   including a JSON config agentic-codebase created, which the un-merge script deletes on a rehearsal copy
   because nothing but agentic-codebase's entries is in it. (On the user's real tree that same file survives
...
git.** The build records it from the file's bytes *before* agentic-codebase touched them, so a file git never
...
that file — the tree, the rehearsal copy, `built.txt` — already has agentic-codebase's contribution in it.
...
while :; do
  ...
  [ -f "$src" ] || src="$HOME/agentic-codebase-removal-steps.md"   # only because block A's step 1 named it
  if [ ! -f "$src" ]; then
...
block from the file, failed at block 2 with `.claude/settings.json` still registering agentic-codebase's
...
- **anything about the user's tree at the moment they actually run the undo.** The copy is untouched
  ...
  *deleted* here; a file they have edited since takes the marker script's `WARNING … only the
  agentic-codebase block was removed` path, which the rehearsal never exercises. Both are correct behaviours
...
group and a `Stop` group, with agentic-codebase's `enforce-bun` command merged into that same `Bash` group:

```
# the marker script — the shipped defect
SKIP  .claude/settings.json: no agentic-codebase block id=enforce-bun (already removed by hand?)
...
And on a second fixture where agentic-codebase **created** `.claude/settings.json`:
`deleted (agentic-codebase created it and it now holds nothing else)`, then `rm -f` the hook script and
...
| `.claude/settings.json` | `uninstall` | fail | `the removal block hands this JSON config to the MARKER script, which finds no \`agentic-codebase:begin\` in a file that can never hold one, prints SKIP and exits 0. It belongs in the un-merge script's UNMERGE list (the axios defect)` |
...
part of a real agentic-codebase run and do not establish hook trust or live custom-agent loading.
