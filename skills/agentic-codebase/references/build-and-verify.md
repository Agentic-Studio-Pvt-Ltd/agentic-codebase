...
Check with `wc -l skills/agentic-codebase/SKILL.md` before you commit an edit to it.
...
read.** agentic-codebase does not emit a plugin manifest on either target (`adapters/capabilities.md`); the
...
`agentic-codebase:begin` / `agentic-codebase:end` is the only pair anything emits; `templates/index-doc-section.md.tmpl`
defines it. `verify_artifacts.py` also recognises `agentic-codebase:start`, **only** so that a rerun can find
and rewrite a block an older agentic-codebase wrote. That alias is backward compatibility, not a second legal
...
**A marker does not by itself mean block B.** The `[table]` block agentic-codebase appends to
`.codex/config.toml` carries the same `agentic-codebase:begin` / `agentic-codebase:end` pair in `#` comments, and its
...
### 2.2 Where each type lands, and **how** agentic-codebase writes it — per target
...
| 1 | rule — command policy | *(none — a `PreToolUse` hook does this job)* | `.codex/rules/agentic-codebase.rules` · Starlark · **create** |
...
| 3 | permissions | `.claude/settings.json` · JSON · **merge** (§2.4a), a second merge into the same file | *(none — folded into `.codex/rules/agentic-codebase.rules` at step 1)* |
...
1. **Project trust gates the whole repo-scoped layer.** `AGENTS.md`, `.codex/hooks.json`,
   ...
   `[projects."<abs repo path>"] trust_level = "trusted"`. Agentic Codebase never edits that file. Untrusted
...
Every byte agentic-codebase puts on disk goes in by exactly one of these, and the mode is recorded per
...
| **append-marked** | appends one `agentic-codebase:begin` / `agentic-codebase:end` block to a text file that did exist | `action: modified`, no `merge`, `pre_existing_sha256`, `restore` | the marker script, block B |
...
2. Parse. **Does not parse ⇒ stop.** Never repair, never overwrite. Write the intended content to
   `<path>.agentic-codebase-proposed` and record a needs-you item.
...
7. **Write no metadata key.** `templates/settings-hooks.json.tmpl` step 6 forbids merging `_agentic-codebase`
   ...
   (`adapters/codex.md` §4.3). `.mcp.json` is the one file that does carry `_agentic-codebase`. The identity
...
8. Record `merge: {"strategy": "json-entries", "entries": [["hook_command", <the registered command
   ...
   **A `hook_command` entry carries a third element, the event it was registered under.** The same
   ...
   registration is agentic-codebase's; the undo needs both. A two-element entry written by an older run still
...
4. Normalise the file to end in exactly one newline, then append `"\n" + block`, where the block is
   **one complete `[table]` block, header first**, wrapped in `# agentic-codebase:begin id=<id>` /
   `# agentic-codebase:end id=<id>` comment markers. **Never append a bare key/value pair**: a `[table]`
...
7. Record `format: "toml"`, `restore: span`, `pre_existing_sha256`, and
   `merge: {"strategy": "toml-table", "marker_id": "<agentic-codebase-id>", "entries": [["toml_table",
...
**(d) Marked text — every other file agentic-codebase adds to.** One `agentic-codebase:begin` / `agentic-codebase:end` block,
...
git hook — anything with a comment syntax that agentic-codebase appends to rather than merges into. **A TOML
...
The dirty-tree status excludes `$PLAN_DIR/` because all of it is agentic-codebase's own output — `plan.md`
...
they may push or merge, and agentic-codebase may not rewrite history to take it back out (`CLAUDE.md` forbids the
...
surprise in someone's PR. This is settled; do not re-open it per run. If
...
base tip has moved (a `CLAUDE.md` violation, caught in `verification.md` §10.2) the commit is the value
...
| `pushed` / `pr_url` | **never set.** agentic-codebase does not push and does not open a pull request (`SKILL.md` phase 8 step 6). The keys are read on a rerun so a branch an older agentic-codebase pushed can still be cleaned up; nothing in this version writes them |
...
with counts, byte-identical to the file's own `agentic-codebase-evidence`). `command` is required on hooks
when the registered command line differs from `path`; `name` is optional.

Added by agentic-codebase for the undo, per artifact and never once per run:
...
| `merge` | every entry agentic-codebase **merged into** — §2.3's third row, and only that row | the object below. **Absent** on a create, a draft, and an append-marked text file: absence is what routes a file to `rm` or to the marker script |
...
| `entries` | `[kind, value]` pairs — except `hook_command`, which is `[kind, value, event]`. Each kind is the identity the emitter actually leaves behind: `hook_command` (the registered command line **plus the event it was registered under** — the whole of agentic-codebase's mark in `settings.json` and in `.codex/hooks.json`, both of which forbid a metadata key; the command alone is not an identity, because the same script can be wired under two events), `mcp_server` (a server name under `mcpServers`), `toml_table` (one per `[table]` header inside the appended block, in file order) |
| `marker_id` | `toml-table` only: the id in the `agentic-codebase:begin` / `agentic-codebase:end` comment pair. Same string as `agentic-codebase-id` |
...
1. **It is written by the step that does the merging, not reconstructed later.** `pre_existing_sha256`
   ...
   splice.
2. **One artifact entry per file agentic-codebase changed**, not per candidate. A hook is two entries — the
...
**`plugin` stays in the enum and nothing emits it.** agentic-codebase no longer builds a plugin manifest
(`adapters/capabilities.md`); the value is kept so a rerun over a repo an older agentic-codebase wrote into
...
| `.codex/config.toml` | `mcp` | `toml` | the only thing agentic-codebase ever writes into it is an MCP block |
...
| `.codex/rules/agentic-codebase.rules` (permissions) | `rule` | `starlark` | on Codex the permission surface **is** the command policy — one file, one entry, never a second `settings` row |
...
### 5.5 agentic-codebase's own three entries, and when each is added
...
- **`plan` and `manifest` are written now, at build time.** Without them the undo leaves both behind,
  and a manifest nothing checks is how a run once shipped with no `agentic-codebase-id` in it at all.
- **The manifest may list itself.** Its own checks are existence, non-emptiness, valid JSON and the
  `_agentic-codebase` identity — none of which depends on its content — so appending an entry afterwards does
...
`_agentic-codebase` is the inert top-level identity object, carrying `agentic-codebase-id: build-manifest`,
`agentic-codebase-version`, `agentic-codebase-generated`, `agentic-codebase-evidence` and a `note` ending
...
`agentic-codebase-id: <kebab-name>`, `agentic-codebase-version: 1`, `agentic-codebase-generated: <yyyy-mm-dd>`,
`agentic-codebase-evidence: <one line with counts>`, and the line `Safe to delete or edit.` — in frontmatter,
...
6; `evidence_match` is a FAIL when the three differ. A rerun matches on `agentic-codebase-id` and rewrites in
...
| a config agentic-codebase merged into has no `merge` record | fix it before phase 8 — an entry with no `format`/`merge` reaches `rm` or the marker script, and both are wrong for it (§5.3a) |
...
| `.codex/hooks.json` does not parse, or holds an unknown top-level key | stop on that file (§2.4a step 2). An unknown top-level key makes Codex load **zero hooks with zero warnings**, so never add one — not `_agentic-codebase`, not anything |
