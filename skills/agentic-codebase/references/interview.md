...
8. **There are no caps, and the interview never mentions one.** agentic-codebase builds the whole setup
...
|---|---|
| *"When I'm done, open a pull request, or leave the branch for you?"* | **agentic-codebase never opens a pull request.** `open_pr` is gone from the record, phase 8 has no PR step, and the push-access ladder that guarded it is gone with it. The branch is left for the user, every time, and the report says so |
...
Those four asked for things that should not exist. These three asked for things agentic-codebase can work
...
| `PERMISSIONS_FILE` | `.claude/settings.json` (`permissions.allow` / `.deny`) | `.codex/rules/agentic-codebase.rules` — the Starlark command policy is this target's permission surface |
...
- **No question is skipped because of the target, and none is left that could be.** Q9 was the last
  ...
  question on both targets (§4.5). Every artifact type agentic-codebase builds has a native home on both
...
|---|---|
| **Q2 — open a pull request?** | The whole **push-access ladder**: reading the push remote, testing the host for GitHub, `command -v gh`, `gh auth status`, and `gh repo view --json viewerPermission`. That ladder was the only network call in a run other than `mine_git.py --no-gh`. **agentic-codebase now makes no network call at all in phases 0–8**, and `SKILL.md`'s no-network rule has one exception rather than two. Phase 8 has no PR step and `open_pr` is not a field |
...
| **Q4 — which services do you touch?** | services ranked structurally, all proposed, most critical first (§4.8) | On a run with no `tool_mentions[]` it hands the user a list of eight and asks them to do the ranking — the work agentic-codebase is for. The plan gate already lets them drop any of them by number |
...
not from a user saying "I'll PR this later". agentic-codebase leaves the branch and says so.
...
| Q12 | `...index_docs[].path` **and** `.bytes`, `.has_agentic_codebase_section`; on Codex `...codex.index_doc_chain_bytes` and `.claude_md_fallback` | `discover.py` | **repointed** — the trigger read any index-doc row, so on a Codex run a large `CLAUDE.md` fired a question about a file Codex does not read. Rows are de-duplicated by real file and carry `path`, `bytes`, `has_agentic_codebase_section`, `is_symlink`, `link_target`, `aliases` — and **no target field**, which is why the `path` test is the fix. All five fields confirmed 2026-09-05 |
...
**not** in the interview. **Do not add a "shall I mark this project trusted?" question**: agentic-codebase
...
- A repo **with** a `docs/` tree is the case that needs the question. That directory is the user's,
  it very often has a build behind it, and agentic-codebase's plan is not their documentation.
...
**Three hard limits.** agentic-codebase never installs a marketplace plugin on its own; never adds a
...
`agentic-codebase-evidence` line says.
...
- **Exactly one option per question is marked recommended**, with a leading `→ ` on its line and a
  ...
  line the eye lands on. There is no unmarked question: a question agentic-codebase cannot recommend an
...
**Three fields are gone and must not be reintroduced:** `open_pr` (agentic-codebase never opens a pull
