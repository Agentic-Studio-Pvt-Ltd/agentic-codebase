...
permitted write. `plan.md` has **no `agentic-codebase:begin`/`agentic-codebase:end` markers and needs none** — it
is agentic-codebase's own document, not a file shared with the user — so if a `plan.md` already exists from
an earlier run, **regenerate the file wholesale, preserving its `agentic-codebase-id`** (`agentic-setup-plan`)
...
first session. This is the contract's secondary user and the commonest fresh-repo run — it is a normal,
...
~~~markdown
---
agentic-codebase-id: agentic-setup-plan
...
`agentic-codebase-id` markers, so a rerun updates them in place instead of duplicating them.
...
occurrences, or a signal that has gone quiet. Per `CLAUDE.md` they are listed and not built. Say the
...
sections inside a file you already had are rewritten between their `agentic-codebase-id` markers, so
anything you edited outside those markers is left alone; my own documents — this plan, `report.md`,
`build-manifest.json` — carry no markers and are regenerated whole under the same `agentic-codebase-id`.
...
conditional on maturity.** There is no audit-only mode , so an existing setup — however
...
- **Modifies:** `CLAUDE.md` (one line in the agentic-codebase section pointing at the skill)
