...
# -*- coding: utf-8 -*-
"""
agentic-codebase :: scripts/discover.py  --  phase 1, the repo analyzer.
...
Design constraints (build contract):
...
* Codex USER scope (`${CODEX_HOME:-~/.codex}` plus `~/.agents/skills`) is
  ...
  into `counts` or `maturity`: agentic-codebase builds repo-scoped files, and a global
...
CLOCK_EVERY = 200
AGENTIC_CODEBASE_MARKER = "agentic-codebase:begin"
...
#: skills root agentic-codebase WRITES is `.agents/skills` -- the documented,
...
#: unused in the wild, which is why agentic-codebase does not write there; skills
#: found there are real artifacts that must be de-duplicated against, and the
#: warning says which path agentic-codebase uses and why rather than calling theirs
...
#: gets an agentic-codebase section appended to a file Codex never reads.
...
# and agentic-codebase refused to build one.  Vendored documentation is not evidence
...
#      mature setup *for this repo*, and agentic-codebase builds repo-scoped files.
...
#: because it is not the path agentic-codebase WRITES.  The roots table Codex 0.152.1
...
#: names the path agentic-codebase writes and the reason.
...
def parse_codex_config_facts(text, repo_root=""):
    """
    The handful of facts agentic-codebase needs out of a Codex `config.toml`, read
...
def scan_codex_user_scope(repo_root, reader, warnings):
    ...
    Why this exists.  Everything agentic-codebase builds is repo-scoped, but Codex
...
#: `.agents/skills/` (the repo skills root agentic-codebase writes) had no row at all,
...
def detect_agentic_config(root, state, reader, warnings):
    ...
    # answers, and appending the agentic-codebase section to each one appends it TWICE
    ...
    for group in doc_groups:
        ...
        if 0 < size <= MAX_INDEX_DOC_BYTES:
            ...
            has_section = AGENTIC_CODEBASE_MARKER in text
        try:
            is_link = os.path.islink(full)
        except (IOError, OSError):
            is_link = False
        index_docs.append({
            "path": canonical,
            "bytes": size,
            "has_agentic_codebase_section": has_section,
            "is_symlink": is_link,
    ...
    #: agentic-codebase writes.  Anything found there is counted for de-duplication
    #: and the user is told which path agentic-codebase uses and why.
    ...
    for rel in sorted(paths):
        ...
        # `$CWD/.agents/skills`) and the one agentic-codebase writes.  It is not the
    ...
    # only on the OTHER target used to be invisible and agentic-codebase would propose
    ...
    # configured" and agentic-codebase proposes a draft for something they have.
    ...
    if "codex" in targets or any(row["target"] in ("codex", "shared")
        ...
        if _codex_skills_dir_used:
            emitlib.warn(
                ...
                "undocumented and unused in the wild, so agentic-codebase writes "
                "repo-level skills to .agents/skills/ instead: the documented "
                "cross-tool root a Claude Code install can share.  That is a "
                "choice about which path agentic-codebase supports, not a verdict on "
        ...
        if chain > 24576:
            emitlib.warn(
                ...
                "files at the cap, so appending an agentic-codebase section can "
    ...
    if ("codex" in targets or any(row["target"] in ("codex", "shared")
        ...
        if codex_block["repo_trust_level"] not in ("trusted", "verified"):
            emitlib.warn(
                ...
                "report's manual checklist; agentic-codebase never edits the user's "
    ...
    # skills root agentic-codebase itself writes to, so an orphan directory there is
...
def build_parser():
    parser = emitlib.base_parser(
        TOOL,
        "agentic-codebase phase 1: walk a repository once and emit discovery.json.",
    )
    parser.add_argument(
...
def selftest():
    ...
    fixture = tempfile.mkdtemp(prefix="agentic-codebase-discover-selftest-")
    try:
    ...
    fixture2 = tempfile.mkdtemp(prefix="agentic-codebase-discover-provenance-")
    try:
    ...
    _hostile_toml = (
        ...
        'OPENAI_API_KEY = "sk-%s"\n'
        '\n[projects."/tmp/agentic-codebase-fixture"]\ntrust_level = "trusted"\n'
    ...
    _facts = parse_codex_config_facts(_hostile_toml, "/tmp/agentic-codebase-fixture")
    ...
    fixture4 = tempfile.mkdtemp(prefix="agentic-codebase-discover-codex-")
    try:
        ...
        with open(os.path.join(fixture4, ".codex", "rules",
                               "agentic-codebase.rules"), "w") as handle:
        ...
        add(
            ...
            (bool(_row("skills", "deploy-api"))
             ...
             and bool(_row("rules", ".codex/rules/agentic-codebase.rules"))
        ...
        add(
            ...
            ".agents/skills as the path agentic-codebase writes",
            (codex4.get("skills_root") == ".agents/skills"
             ...
             and "agentic-codebase writes" in _codex_skills_warns[0]
        ...
        #:     `scope: "repo"` (measured, codex-cli 0.152.1).  agentic-codebase writing
    ...
    outside5 = tempfile.mkdtemp(prefix="agentic-codebase-discover-outside-")
    fixture5 = tempfile.mkdtemp(prefix="agentic-codebase-discover-symlink-")
    try:
    ...
    fixture3 = tempfile.mkdtemp(prefix="agentic-codebase-discover-commands-")
    try:
    ...
    #   acme/web-app       bun.lock at the root, package-lock.json in
    #                               a nested package.
    ...
    def _pm_fixture(label, files):
        ...
        base = tempfile.mkdtemp(prefix="agentic-codebase-discover-pm-")
        try:
    ...
    mono = tempfile.mkdtemp(prefix="agentic-codebase-discover-mono-")
    try:
        def write(rel, body):
    ...
    # -- the 2026-09-07 reference shape: services, zones, docs and samples the
