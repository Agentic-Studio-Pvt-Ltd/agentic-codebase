...
# -*- coding: utf-8 -*-
"""
agentic-codebase :: lib/emit.py -- stdout JSON emission, hard output caps, shared CLI.
...
===========================================================================
  ...
  exit 0  SUCCESS **or DEGRADED**.  Valid JSON on stdout, always.
          ...
          This is the case that matters most.  agentic-codebase runs as a pipeline
...
def fail(
    ...
    # Every agentic-codebase JSON payload carries the same four keys, so a caller can
...
def base_parser(tool_name: str, description: str = "") -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog=tool_name,
        description=description or ("agentic-codebase analyzer: %s" % tool_name),
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="%s (agentic-codebase shared libs, schema_version %d)" % (tool_name, SCHEMA_VERSION),
    )
...
def check_regexes(module: Any, sources: Optional[Iterable[str]] = None) -> Tuple[bool, str]:
    ...
    """
    probe = "agentic-codebase selftest probe 123 /tmp/x.py `npm run build` https://e.co"
    checked = 0
