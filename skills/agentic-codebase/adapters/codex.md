...
**This file is the only place Codex paths and file formats may live**. Same contract,
...
trust gate. See §4.3. Any wording anywhere in agentic-codebase that still says otherwise is stale.
...
**0.3 agentic-codebase never sets trust, and never fakes it either.** Writing
`[projects."<repo>"] trust_level = "trusted"` into `${CODEX_HOME}/config.toml` would make every
phase-8 check above reachable in one line. agentic-codebase does not do it and must never offer to: deciding
that a directory's committed code may configure the agent, run hooks and set an exec policy is a
security decision belonging to the person whose machine it is, and agentic-codebase has no standing to make
...
trees with packfiles. That is a **network call**, out of a phase that promises none. Do
...
3. Otherwise, signals 3–6 prove installation only: **ask the user to confirm the target** (`CLAUDE.md`
...
### 1.4 Model class check (`CLAUDE.md` — warn, never enforce)
...
`thread_history_1.sqlite`, `memories_1.sqlite`, `goals_1.sqlite`, `queue_1.sqlite`. **Agentic Codebase reads
...
evidence alone, and that results improve after a week of usage.
...
| User | `${CODEX_HOME}/AGENTS.md` | global index doc | **DOCUMENTED** as read (first non-empty of `AGENTS.override.md` then `AGENTS.md`). Absent on this machine. Agentic Codebase never writes it. |
...
| User | `${CODEX_HOME}/hooks.json` | hooks | **VERIFIED** to load as `source: "user"`. Agentic Codebase never writes it (§4.3). |
| Project | `<repo>/.codex/rules/*.rules` | command-approval policy | **VERIFIED** — any `*.rules` filename is picked up, not just `default.rules` |
| User | `${CODEX_HOME}/rules/*.rules` | command-approval policy | **VERIFIED** to exist and to be auto-generated by Codex when a user approves a command prefix. Agentic Codebase never writes it. |
...
on by default at 0.152.1. **Agentic Codebase must not write `[features] hooks = true`**; it is unnecessary
...
permissions step and the rules step produce the **same file** — `.codex/rules/agentic-codebase.rules` is
...
Everything else has a native, repo-scoped, committable home. Any wording elsewhere in agentic-codebase that
...
`agentic-codebase-id` / `agentic-codebase-version` / `agentic-codebase-generated` / `agentic-codebase-evidence` block and the line
...
point of the repo-scoped `.codex/` layer is that agentic-codebase never needs to.
...
| Global variant | `${CODEX_HOME}/AGENTS.md` — **DOCUMENTED** as read. Agentic Codebase never writes it: a global index doc applies to every repo on the machine, which contradicts evidence-derived-for-this-repo. |
...
- **There is no import or include mechanism.** Nothing equivalent to Claude Code's `@path`. Cross-file
  ...
  model-followed, not loaded. Agentic Codebase's pointer approach is therefore the right one here.
...
3. `project_doc_fallback_filenames = ["CLAUDE.md"]` may be offered in the report as a **user-applied**
   config line, never written by agentic-codebase. It is a `config.toml` key and §4.6 explains why agentic-codebase
...
```markdown
<!-- agentic-codebase:begin id=index-doc-section -->
<!--
agentic-codebase-id: index-doc-section
agentic-codebase-version: 1
agentic-codebase-generated: 2026-09-05
agentic-codebase-evidence: <one line, with counts>
Safe to delete or edit.
-->

## Agentic setup
…
<!-- agentic-codebase:end id=index-doc-section -->
```

**The marker spelling is `agentic-codebase:begin` / `agentic-codebase:end`, everywhere, with no synonym.**
`templates/index-doc-section.md.tmpl` emits exactly that, and it is what `SKILL.md` and the plan
template document. `verify_artifacts.py` also accepts `agentic-codebase:start` and a few other spellings, but
only so a rerun can still find a block written by an older agentic-codebase — never emit one.
...
confirmed in a real repo config). Appending agentic-codebase's section to an already-large `AGENTS.md` can
...
- **Short rules (≤ ~15 lines) and anything that must always apply:** inline them in the `AGENTS.md`
  agentic-codebase section under a `### Conventions` heading. The index doc is the only prose load path this
...
- **Long or path-scoped rules:** one file per rule at `<plan-dir>/rules/<kebab-name>.md`, with the
  agentic-codebase ID block in frontmatter, **plus** a pointer line in the `AGENTS.md` section:
...
command prefix; **agentic-codebase never writes it.** The repo-scoped file is fair game.

```starlark
# <repo>/.codex/rules/agentic-codebase.rules
# agentic-codebase-id: forbid-npm
# agentic-codebase-version: 1
# agentic-codebase-generated: 2026-09-05
# agentic-codebase-evidence: 6 corrections across 4 sessions telling the agent to use bun, not npm
# Safe to delete or edit.
prefix_rule(
    pattern = ["npm"],
    decision = "forbidden",
...
| `#` comments | plain-text file, so the agentic-codebase ID block goes in header comments exactly as shown. |
...
1. **Validate before writing into the repo.** Write to a temp path, run
   ...
   is not acceptable.
2. **One agentic-codebase-owned file, never a merge.** Write `<repo>/.codex/rules/agentic-codebase.rules` and own it
   whole. If a file of that name exists without the agentic-codebase ID block, do not touch it — write
   `agentic-codebase.rules.proposed` and record a needs-you item.
...
```json
{
  "description": "Agentic Codebase-generated Codex lifecycle hooks.",
  "hooks": {
    "PreToolUse": [
      {
...
hook agentic-codebase would emit on Claude Code, Codex has an equivalent event.**
...
there is no `Bash` — so agentic-codebase emitted shell matchers that excluded the one name a current Codex
...
| Key | Status |
|---|---|
| `type` | required. `"command"` (**VERIFIED**) and `"mcp_tool"` (**VERIFIED** to load, reported back as `handlerType: "mcpTool"`, requires `server`, `tool`, `input`). The protocol schema also names `prompt` and `agent` handler types. **Agentic Codebase emits `"command"` only** — it is the simplest, the most portable, and the only one testable from a shell. |
...
| `async` | **VERIFIED** to load (`"async": true` → `async: true`). Not emitted by agentic-codebase in v1.1: an async hook cannot block, which is the whole point of the hooks agentic-codebase builds. |
...
2. Parse as JSON. **If it does not parse, stop.** Do not repair, do not overwrite. Write the intended
   content to `<repo>/.codex/hooks.json.agentic-codebase-proposed` and record a needs-you item.
...
7. Record `pre_existing_sha256` (the hash **before** the merge) and `restore: span` on the artifact's
   ...
   alone does not say which registration is agentic-codebase's, and the undo refuses to guess rather than
...
**`hooks.json` rejects unknown keys at the top level.** Measured, with a `_agentic-codebase` object added
beside `hooks`:

```
failed to parse hooks config …/.codex/hooks.json: unknown field `_agentic-codebase`,
...
- **Never write an `_agentic-codebase` block into `hooks.json`.** Unlike `.mcp.json` on Claude Code, there is
...
- Unknown keys *inside a handler object* were tolerated at 0.152.1 (a handler carrying
  `"_agentic-codebase_id"` loaded normally). **Do not rely on that** — the top level is strict, the asymmetry
  is undocumented, and a future build tightening it would brick the file. Agentic Codebase emits no metadata
...
- **The identity is the whole command path, plus the event it is registered under**, exactly as in
  ...
  matched on basename removed both. The un-merge normalizes the repo-root spellings agentic-codebase emits
...
- `description` is an accepted key and agentic-codebase may set it **only when it created the file**. If the
...
#!/usr/bin/env bash
set -euo pipefail
# agentic-codebase-id: enforce-bun
# agentic-codebase-version: 1
# agentic-codebase-generated: 2026-09-05
# agentic-codebase-evidence: 6 corrections across 4 sessions telling the agent to use bun, not npm
# Safe to delete or edit.
...
1. **Explicit approval, asked separately.** Git hooks run on every commit, forever, outside the
   ...
   consent for this.
2. **Never overwrite an existing hook.** If `.git/hooks/pre-commit` exists without an agentic-codebase
   marker, do not touch it: write `.git/hooks/pre-commit.agentic-codebase` (non-executable) and put the
...
4. **Idempotent markers**, since shell has no frontmatter, and the dry-run escape hatch:
   ```bash
   # agentic-codebase:begin id=pre-commit-typecheck
   # agentic-codebase-id: pre-commit-typecheck
   # agentic-codebase-version: 1
   # agentic-codebase-generated: 2026-09-05
   # agentic-codebase-evidence: 9 sessions ended with a failing typecheck; "run typecheck" asked 5 times
   # Safe to delete or edit.
   [ -n "${AGENTIC_CODEBASE_DRYRUN:-}" ] && exit 0     # required: makes the hook smoke-testable
   bun run typecheck
   # agentic-codebase:end id=pre-commit-typecheck
   ```
   The `AGENTIC_CODEBASE_DRYRUN` early exit is **required in every generated git hook** — it is what lets
...
> Skills row now says *not the path agentic-codebase writes*, and gives the real reason (undocumented, unused
...
agentic-codebase-id: add-api-endpoint
agentic-codebase-version: 1
agentic-codebase-generated: 2026-09-05
agentic-codebase-evidence: 7 requests across 5 sessions matching "add endpoint <resource>"
---
<!-- Safe to delete or edit. -->
```
...
| `version`, `allowed-tools`, `user-invocable` | no | **VERIFIED in the wild** on this machine's skills. Agentic Codebase omits them. |
...
name, `name` and `agentic-codebase-id` are all the same string.
...
| Rules — command policy / permissions | `.codex/rules/agentic-codebase.rules` | Starlark `prefix_rule(...)`. **Validate with `codex execpolicy check` before writing — a malformed file bricks Codex in this repo** (§4.2.2) |
...
| Skills | `.agents/skills/<name>/SKILL.md` (+ `references/`) | markdown + frontmatter. `.codex/skills/` loads too but is undocumented, so it is **not the path agentic-codebase writes** (§4.4) |
...
| Format | TOML, one agent per file, authored whole by agentic-codebase — no round-trip, no merge (§4.6 does not apply) |
...
…
'''
# agentic-codebase-id: pr-reviewer
# agentic-codebase-version: 1
# agentic-codebase-generated: 2026-09-05
# agentic-codebase-evidence: 11 review requests across 8 sessions; 34% of commits touch no test file
# Safe to delete or edit.
```
...
| `model` | no | **VERIFIED in the wild.** Agentic Codebase **omits it** — a pinned model goes stale and silently changes the user's session. |
...
**Comments.** TOML has `#` comments, so the agentic-codebase ID block goes in the file as shown. It is a
whole-file artifact agentic-codebase owns; a rerun rewrites it, and a file of that name without the ID block
...
1. The plan's capability note says **what is enforced and by what**, in those words. Never "the
   ...
   prose.
2. agentic-codebase never provisions a credential. If no read-only credential exists for the service, the
...
#### Can agentic-codebase merge a block into `config.toml` programmatically? Mostly no — here is the precise line

Agentic Codebase is **stdlib-only and targets Python 3.9**. `tomllib` is read-only *and* 3.11+. **There is no
...
Emit `<plan-dir>/codex-mcp.toml`, copy-paste ready, with the agentic-codebase ID block in `#` comments, and a
...
```toml
# agentic-codebase-id: mcp-linear
# agentic-codebase-version: 1
# agentic-codebase-generated: 2026-09-05
# agentic-codebase-evidence: "linear" mentioned in 12 user turns across 7 sessions
# Safe to delete or edit.
...
- **The file does not exist.** Writing it is creating a whole file from a template, not editing one —
  ...
  the safe case, and the only TOML file agentic-codebase ever authors from nothing besides
  `.codex/agents/*.toml`.
...
There is no third option: agentic-codebase never hand-edits an existing TOML file beyond appending a whole
...
- **The user authenticates.** Agentic Codebase never runs an auth flow, never launches a server, never tests
  connectivity.
...
agentic-codebase does **not** emit a plugin manifest on either target. The setup is derived from one repo's
...
`capabilities[]`, `websiteURL`, `defaultPrompt[]`, and policy URLs). Agentic Codebase emits the minimal form
...
- **Agentic Codebase cannot restart the user's Codex session**, so anything that needs a fresh session is a
...
| **armed** — Codex will execute it | **never.** `trustStatus` comes back `"untrusted"` on everything agentic-codebase writes, in every project, always. |
...
| Rules (prose, inline) | extract the block first, then count inside it: `awk '/^[ \t]*<!--[ \t]*agentic-codebase:begin/{f=1} f{print} /^[ \t]*<!--[ \t]*agentic-codebase:end/{if(f)exit}' "$REPO/AGENTS.md" \| grep -c '^### Conventions'` | present exactly once. **Never `grep -A400 'agentic-codebase:begin'`**, which was the old form and is wrong twice over: the pattern is unanchored, so it matches the quoted marker in the "Removing this" bullet as well as the real one, and `-A400` then runs straight past `agentic-codebase:end` into the user's own prose. Measured 2026-09-05 on a rendered `AGENTS.md` whose author keeps their own `### Conventions` heading below the agentic-codebase block: the old form counts **2** and fails a correct build; the bounded form above counts **1**. Same family as the index-doc row — a check keyed on a marker string must be both line-anchored **and** bounded by the closing marker. |
...
| MCP / any TOML agentic-codebase wrote — syntax | parse it with `python3 -c "import tomllib,sys;tomllib.load(open(sys.argv[1],'rb'))" <file>` | parses ⇒ pass. **Always reachable, and on a first run it is the only TOML check that is** — so it is the pass condition. A parse failure ⇒ restore the pre-write bytes from `pre_existing_sha256` and record a needs-you item. |
| MCP / any TOML agentic-codebase wrote — field names — **trust-gated** | start `"$CODEX" --strict-config app-server` from the repo (the same process the probe below already starts) and send `initialize` | **VERIFIED** on a trusted project: with a valid config it stays alive and answers `initialize`; with an unknown field it exits 1 and prints `Error: <file>:<line>:<col>: unknown configuration field …` on stderr. It makes no model call and needs no auth. **But the repo-scoped `<repo>/.codex/config.toml` is behind the same trust gate as everything else** — measured: with no `[projects]` entry, an unknown field in it does **not** stop the server. So on an untrusted project a clean start proves nothing about the file agentic-codebase just wrote: record `not tested — project not trusted` and do **not** report it as validated. A failure in a trusted project ⇒ restore `pre_existing_sha256` and record a needs-you item. |
| Plugin manifest | **never runs** — agentic-codebase emits no plugin manifest on either target (§4.7), so no run produces one to check. The row is kept only so that a repo where the *user* already has a `.codex-plugin/plugin.json` is not mistaken for agentic-codebase's output: that file belongs to them and phase 8 leaves it alone | n/a — the artifact type does not exist in a build manifest |
| `${CODEX_HOME}/config.toml` not modified **by agentic-codebase** | hash it before and after the build; **on a mismatch, diff the section headers and key names** (never the values) | the set of section headers and keys is unchanged. **A plain hash comparison is not a valid check on its own**: the ChatGPT desktop app and a background `codex app-server` daemon write to this file on their own — its mtime changed twice during verification with no write from here. A structural mismatch is a build failure (§4.6 rule 1); an mtime or hash change with identical structure is the desktop app, not agentic-codebase. |
...
send({"id": 1, "method": "initialize",
      "params": {"clientInfo": {"name": "agentic-codebase", "version": "1"}}})
time.sleep(1.0)
...
- **`trustStatus": "untrusted"`** on everything agentic-codebase wrote — expected, always, in every project.
  It is the *armed* question (§5.0), not a failure.
- **`source`** should be `"project"`; `"user"` means something wrote to the global file, which
  agentic-codebase must never do.
...
1. **Trust the project.** If `${CODEX_HOME}/config.toml` has no `[projects."<abs repo path>"]` entry
   ...
   nothing, silently, and agentic-codebase does not set it because whether a repo's committed files may
...
2. **Trust the hooks.** Run `/hooks` in a Codex session and approve the new hooks. Reason: they are
   ...
   agentic-codebase writes, in every project. Editing a hook re-arms this prompt. Never suggest
   `--dangerously-bypass-hook-trust`.
3. **Confirm the index doc is being followed.** Start a **new** session in this repo and ask a
   question whose answer is in the agentic-codebase section (e.g. "what package manager does this repo
   use?"). Reason: agentic-codebase cannot restart the user's session, so this is the first turn that can
...
5. **MCP.** Export the listed environment variables, append the server block from
   `<plan-dir>/codex-mcp.toml` to `<repo>/.codex/config.toml` if agentic-codebase did not, restart Codex,
   and confirm the server's tools appear. Reason: the block references each variable **by name** and
   the value never goes in a file; agentic-codebase never authenticates a server (§13).
   `codex doctor --json` reports a warning naming any server whose environment variables are
   missing.
6. **Removal.** Delete the branch, or delete the listed files, the `agentic-codebase:begin`/`agentic-codebase:end`
   blocks in `AGENTS.md`, the agentic-codebase handler objects in `.codex/hooks.json`, and any `.git/hooks/`
   entries between agentic-codebase markers.
...
- [ ] Does `hooks.json` still reject unknown **top-level** keys, and does it still tolerate unknown
      **handler** keys? The asymmetry is undocumented; agentic-codebase relies on neither, but a change to the
...
- [ ] Does `${CODEX_HOME}/prompts/` exist as a supported custom-prompt location? It does not exist on
      the verification machine and agentic-codebase never writes there.
- [ ] Can hooks also be declared in TOML (`[hooks]` in `config.toml`)? **DOCUMENTED** as using the same
      event schema; agentic-codebase prefers `hooks.json` because JSON is stdlib-writable and TOML is not.
...
- [ ] Run the full pipeline against a range of real repositories with `--target codex` and diff the
