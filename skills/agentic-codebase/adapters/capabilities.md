...
at it) · **Draft** (written for the user to apply; agentic-codebase never authenticates) · **Substituted**
...
| Permissions | **Native** — the `permissions` object in `.claude/settings.json`: `allow` / `ask` / `deny`, `deny` wins **[D+M 2.1.260]** | **Native** — `.codex/rules/agentic-codebase.rules`, the same Starlark file the command-policy rows write; there is no separate permissions block **[D+M 0.152.1]** |
| Plugin manifest | **Not emitted** — agentic-codebase builds one repo's personalized setup, which is not portable by construction. Both targets support plugins; agentic-codebase does not produce one **[D]** | **Not emitted** — same reason **[D]** |
...
phase 8; agentic-codebase never edits `config.toml` to set it.
...
exists. **`[features] hooks = true` is NOT required** and agentic-codebase must never write it: hooks are on
...
permission surface *is* `.codex/rules/agentic-codebase.rules`, so the permissions candidate and the
...
**agentic-codebase itself being distributed as a Claude Code plugin** is how its own install route 1 works
...
identified by its TOML `name`; matching its filename is agentic-codebase’s generation convention.
