...
outside this file, you are breaking `CLAUDE.md` — move it here instead.
...
**Ambiguity rule.** If signals 1–3 are absent and both `~/.claude/` and `~/.codex/`
...
`/private/tmp/agentic-codebase_verify.enc test/sub` — a path holding an underscore, a dot **and** a space
— and Claude Code created `~/.claude/projects/-private-tmp-agentic-codebase-verify-enc-test-sub`, whose
first `user` record carries `cwd: "/private/tmp/agentic-codebase_verify.enc test/sub"`.
...
| `_` | `-` | **VERIFIED** (probe) — this was the long-standing UNVERIFIED marker. `agentic-codebase_verify` → `agentic-codebase-verify`. Settled. |
...
```
/Users/x/agentic-codebase
  -> -Users-x-agentic-codebase
/Users/x/Documents/projects/mobile-apps/Comic-app
  -> -Users-x-Documents-projects-mobile-apps-Comic-app
/private/tmp/agentic-codebase_verify.enc test/sub
  -> -private-tmp-agentic-codebase-verify-enc-test-sub
```
...
> | `/private/tmp/agentic-codebase_verify.enc test/sub` | `…-enc test-sub` | `…-enc-test-sub` | 0.57 | **fuzzy** — right answer, but it emits "confirm with the user before trusting counts" |
...
1. **Prefix sweep.** Sessions started in a *subdirectory* of the repo get their own directory
   (**VERIFIED** (artifact): `...-acme-web-app` and
   `...-acme-web-app-apps-web-src-app` both exist). Collect the exact directory **plus**
   every directory whose name begins with `<encoded>-`. Require the following character to be `-`
   so `agentic-codebase` does not swallow `agentic-codebase-web`.
...
- **Never read the `memory/` directory** that sits beside those `*.jsonl` files, nor
  ...
  is model-written persistent memory about the user's work — it is not a transcript, agentic-codebase has
...
| Project | `<repo>/.claude/output-styles/`, `workflows/`, `monitors/`, `themes/`, `.lsp.json` | other config agentic-codebase does not emit | **VERIFIED** (schema) — list them so the report can say what exists; **never write to them** |
...
- **Never overwrite.** Create, or update strictly between agentic-codebase markers — **except for the two
...
- Every generated file carries the ID block from the build contract:
  `agentic-codebase-id`, `agentic-codebase-version: 1`, `agentic-codebase-generated: <yyyy-mm-dd>`,
  `agentic-codebase-evidence: <one line with counts>`, and the line `Safe to delete or edit.` — again except
  `settings.json`, whose merged entry carries none of it (§4.3); `.mcp.json` carries it in a
  top-level `_agentic-codebase` object (§4.6).
...
| Format | Plain Markdown. No frontmatter required or expected. A file over 4 MiB is skipped outright (**VERIFIED** (schema)) — not a limit an agentic-codebase section will reach, but it is why an oversized `CLAUDE.md` can appear to load nothing at all. |
...
plan template document. `verify_artifacts.py` also accepts `agentic-codebase:start` and a few other
spellings, but only so a rerun can still find a block written by an older agentic-codebase — never
...
| Format | Markdown, with the agentic-codebase ID block in YAML frontmatter |
...
- **Wiring is still worth doing, for a different reason.** A one-line pointer in the `CLAUDE.md`
  agentic-codebase section makes the rule *discoverable by a human* reading the repo, and it survives the
...
paths:
  - "src/db/**"
  - "src/api/**"
agentic-codebase-id: db-access-through-repository
agentic-codebase-version: 1
agentic-codebase-generated: 2026-09-05
agentic-codebase-evidence: corrected 4 times across 3 sessions; 61 commits touch src/db/
---
<!-- Safe to delete or edit. -->
...
- Unknown frontmatter keys are ignored, so the `agentic-codebase-*` ID block sits in the same frontmatter
...
**Agentic Codebase emits only the eight below.** The rest exist, are legal, and must be *preserved* on
...
verification machine). Preserve them on merge, as before; agentic-codebase still does not emit them,
...
exist — `command`, `prompt`, `mcp_tool`, `http`, `agent` (**VERIFIED** (schema)). **Agentic Codebase emits
...
| `args` | **NEW, and worth knowing.** An argument list. When present, `command` is resolved as an executable and spawned **directly, with no shell**, and path placeholders are substituted per element as plain strings — so a path containing a quote, `$` or a backtick never reaches a shell parser. Agentic Codebase does not emit it in v1 (the shell form is what every existing hook in the wild uses), but it is the correct answer if a generated hook ever needs to take a filename as an argument. |
| `timeout` | **Seconds** (**VERIFIED** (schema)). See the default table above. |
| `statusMessage` | **No longer UNVERIFIED** — it is an official optional key, "Custom status message to display in spinner while hook runs" (**VERIFIED** (schema)). Agentic Codebase still omits it: it is cosmetic, and every character of it is context the user pays for on every fire. |
| `if` | A permission-rule pattern (e.g. `Bash(git *)`) that gates whether the hook process is spawned at all. Cheap and precise. Agentic Codebase does not emit it in v1 — the matcher plus an early exit in the script covers the same ground and is visible in one file — but note it in the plan if a user asks why their hook runs on every Bash call. |
...
**Agentic Codebase still emits exit-code hooks only**, and the reason is now a choice rather than an
...
- Reference it from settings as **`"${CLAUDE_PROJECT_DIR}/.claude/hooks/<name>.sh"` — with the
  ...
  variable and substitutes nothing. **The braced form is the portable spelling and is what agentic-codebase
...
2. Parse it as JSON. **If it does not parse, stop.** Do not repair it, do not overwrite it. Write
   the intended content to `<repo>/.claude/settings.json.agentic-codebase-proposed` and record a
...
JSON has no comment syntax, so this file carries **no `agentic-codebase:begin` / `agentic-codebase:end` markers and
no `# agentic-codebase:<id>` line**, and it never can. What agentic-codebase emits into it is one command object and
...
> PowerShell reason is in the bullet list above), but a setup built by an older agentic-codebase carries the
...
**Do not merge the fragment's `_agentic-codebase` key into `settings.json`** (template step 6). It is inert
metadata describing the fragment, not settings, and this is the one `_agentic-codebase` block that is never
written to a generated file. Provenance in the merged file lives in the command path and in the
header of the hook script it points at. `settings.json` therefore gets no `agentic-codebase-id`, no
...
agentic-codebase merged into is `restore: span` — the user wrote it, git never tracked it — and it is
`action: modified`, which past wording routed to the marker-removal script. That script looks for a
marker this file cannot contain: it prints `SKIP .claude/settings.json: no agentic-codebase block id=…`,
...
An older agentic-codebase wrote `"# agentic-codebase:<id>\n<path>"` as the command. Nothing emits that any more —
...
backward compatibility, exactly like the `agentic-codebase:start` marker alias, and not a second legal
spelling.
...
**What agentic-codebase emits** — three keys and the ID block, and that is the whole list:
...
| `allowed-tools` | no | **Settled — this was UNVERIFIED and is not any more.** The bundle's schema describes it as "Tools available to the model while this file is active. **Comma-separated string or YAML list.**" Both spellings are canonical (**VERIFIED** (schema)); the space-separated form seen in the wild is not. Agentic Codebase **omits it by default** (the skill inherits the session's tools). Emit it only if the interview asked for tool restriction, and then use the **comma-separated string** form. The "spelling is unverified" caveat comes out of the plan. |
| `version` | no | **VERIFIED** (artifact) in the wild as an optional string; the bundle marks it internal bookkeeping. Agentic Codebase omits it; `agentic-codebase-version` covers it. |

**What 2.1.260 also accepts, and why agentic-codebase does not emit it.** The frontmatter surface has grown
...
- **`paths`** — glob patterns; "the skill only loads when the model touches matching files". The
  ...
  v1, which is why agentic-codebase does not reach for this yet, but it is the obvious v1.1 sharpening.
...
| Surface | Path | Format |
|---|---|---|
| Index doc | `CLAUDE.md` | markdown; agentic-codebase's part lives between `agentic-codebase:begin`/`agentic-codebase:end` |
...
`agentic-codebase-id`, and what each artifact governs — which is why it is the **last** skill written.
...
| `model` | no | **Settled — the value set was UNVERIFIED and is not any more.** The bundle documents it as: a model alias (`fable`, `opus`, `sonnet`, `haiku`), a full model ID (e.g. `claude-fable-5`), or `inherit`; **if the key is omitted** the agent uses the configured default subagent model, and failing that the main model (**VERIFIED** (schema)). Agentic Codebase emits `inherit`, or omits the key entirely. Do not pin a named model in a generated file — it goes stale and it silently downgrades the user's session. |

**What 2.1.260 also accepts, and why agentic-codebase does not emit it.** `disallowedTools`,
...
worth naming so nobody adds them casually: **`memory`** turns on persistent state that agentic-codebase's
...
server name agentic-codebase adds at the repo root can be shadowed by a nested `.mcp.json` the user
...
**The user authenticates — the tool never does.** Agentic Codebase does not run `claude mcp add`, does not
start an OAuth flow, does not launch the server, and does not verify connectivity. It
...
file carries no `agentic-codebase:begin` / `agentic-codebase:end` pair and never can. **A key inside `mcpServers`
...
`_agentic-codebase`, carrying `agentic-codebase-id`, `agentic-codebase-version`, `agentic-codebase-generated`, `agentic-codebase-evidence`,
a `note` ending `Safe to delete or edit.`, `status`, `needs_you` and `remove`. Emit it, at the top
level only. *(An earlier version of this section banned `_agentic-codebase` outright while the template
...
`_agentic-codebase` block whose `agentic-codebase-id` matches. List the servers in the `CLAUDE.md` agentic-codebase section
as well.
...
`mcpServers` name this run added and drops the `_agentic-codebase` object only when its `agentic-codebase-id` matches
the artifact's, leaving every server the user added by hand untouched; it deletes the file outright
only when agentic-codebase created it and nothing else is left in it. Note the asymmetry with §4.3 and keep
it straight: **`.mcp.json` carries `_agentic-codebase`; `settings.json` does not**, so the un-merge keys on a
...
4. **Never remove or reorder an entry agentic-codebase did not add.** If a user `allow` conflicts with a
...
agentic-codebase does **not** emit a plugin manifest. The setup it builds is derived from one repo's
...
That resolves the question but does not change what agentic-codebase builds: **a packaged plugin still
...
|---|---|---|
| Index doc | `test -f "$REPO/CLAUDE.md" && grep -cE '^[ \t]*<!--[ \t]*agentic-codebase:begin' "$REPO/CLAUDE.md"` | file exists; marker count is exactly `1`. **The grep must be line-anchored.** `index-doc-section.md.tmpl` quotes both marker strings inside backticks in the section's own "Removing this" instructions, so a plain `grep -c 'agentic-codebase:begin'` counts **2** on a correctly emitted block and this check fails on every run — measured 2026-09-05 on the Codex adapter's identical row. `verify_artifacts.py`'s `_MARKER` regex is line-anchored for the same reason. **Count the `end` marker the same anchored way and require `1` too** — `grep -cE '^[ \t]*<!--[ \t]*agentic-codebase:end' "$REPO/CLAUDE.md"` — and require the begin line to come first. Anchoring makes both counts right, but the two markers are not protected by the same thing, and only one of them is safe by construction: the quoted `begin` sits mid-sentence after `- This section: delete everything from `, so the anchor rejects it however the bullet is formatted; the quoted `end` **opens** its line and is rejected only because the template wraps it in a backtick. Measured 2026-09-05 on a rendered block — strip the backticks out of that bullet and the anchored `begin` count stays `1` while the anchored `end` count goes to `2`. Those backticks in `index-doc-section.md.tmpl`'s "Removing this" bullet are therefore load-bearing for this row *and* for `verify_artifacts.py`'s `marker_block()`, which would truncate the block at the quoted marker; if that bullet is ever reflowed or unquoted, both break together. On the same fixture `find_markers()` returns exactly two markers, one `begin` and one `end` — that is the count this row has to agree with. Then every `@import` and relative link target **inside the block** resolves — extract the block first, the same bounded way the Codex adapter's inline-rules row does, and only then scan it: `awk '/^[ \t]*<!--[ \t]*agentic-codebase:begin/{f=1} f{print} /^[ \t]*<!--[ \t]*agentic-codebase:end/{if(f)exit}' "$REPO/CLAUDE.md" \| grep -oE '@[^ ]+\.md' \| sed 's/^@//' \| xargs -I{} test -f "$REPO/{}"`. **Scanning the whole file is a false failure waiting to happen**: a dangling `@import` the user wrote above the block is not agentic-codebase's, and it fails this row on an otherwise correct build — measured 2026-09-05 on a fixture with one user-authored `@notes/missing.md` above a correctly emitted block, where the unbounded pipeline exits non-zero. Bound every marker-keyed check to the block, on both adapters. Manual follow-up in the report: open a new session and run `/memory` to confirm the section is loaded. |
...
(delete the branch, or delete the listed files and the `agentic-codebase:begin`/`agentic-codebase:end` blocks).
...
- [ ] Run the full pipeline end-to-end against a range of real repositories and diff the artifact
...
- [ ] **UNVERIFIED — is a `paths:` glob on a rule case-sensitive?** The match is gitignore-style
      ...
      build if a glob matches no tracked file, and agentic-codebase derives globs from real repo paths
...
- [ ] **UNVERIFIED — is a hook `matcher` a full match or a search?** i.e. does `matcher: "Edit"`
      ...
      call to each tool. **Guard until then:** agentic-codebase only ever emits exact tool names and plain
      ...
      agentic-codebase builds — it changes what the report may promise about a hook the *user* writes.
