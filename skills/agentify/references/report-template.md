...
3. **"Needs you" items are numbered, one step per line, copy-pasteable, and each carries the reason
   agentify stopped there.** No "configure your credentials" hand-waving — name the file, the key,
   and the command. The section covers two things, not one: credentials agentify must never hold,
...
5. **The removal section is generated from `build-manifest.json`, never copied from section 3.1
   ...
   every appended-to file with its `agentify-id`. Someone must be able to undo this run without
   ...
   `SKIP .claude/settings.json: no agentify block id=…`, **exits 0**, and removes nothing, because
   ...
   `.codex/config.toml` matched no rule, reached no mechanism, and the hook agentify registered in
...
8. **`report.md` carries no `agentify:begin`/`agentify:end` markers and needs none.** Markers
   govern files agentify *shares* with the user; `plan.md`, `report.md` and `build-manifest.json`
   are agentify's own documents. On a rerun, **regenerate this file wholesale, preserving its
   `agentify-id`** (`agentic-setup-report`) — one report, updated in place. Never append a second
...
~~~markdown
---
agentify-id: agentic-setup-report
agentify-version: 1
...
Files modified (appended inside `agentify-id` markers, never overwritten):
...
Run the skill again whenever. Each generated file carries an `agentify-id`; a rerun updates the
...
take out only what agentify put in: the block between the `agentify:begin` / `agentify:end` markers
in a text file, the entries agentify merged into a JSON config, which carries no markers because JSON
has no comments, or the one marked `[table]` block agentify appended to a TOML config. Rerunning
agentify updates that same block or entry in place instead of adding a second one.
...
| `EVIDENCE_ONE_LINER_WITH_COUNTS` | the frontmatter `agentify-evidence` line — one line with numbers describing **this file's own** contribution, e.g. `handoff for the 12 artifacts built and 4 skipped`. It is not the run's evidence summary and not `plan.md`'s line: `verify_artifacts.py`'s `evidence_match` requires it to be the **same string** as the `evidence` field on `report.md`'s own manifest entry (`verification.md` §9 step 6 shows that entry), and a drift between them is a FAIL. Missing or empty is a `metadata` FAIL on its own |
...
| `MODIFIED_ID` | the artifact's `agentify-id`, the same string in its begin/end markers |
...
| `REMOVAL_PR_NOTE_OR_DELETE` | **always delete this token.** agentify never opens a pull request and never pushes (`SKILL.md` phase 8 step 6, `interview.md` §4.1), so there is never a remote branch for the removal steps to reach. The token is kept only so a rerun over a repo an older agentify pushed from can still emit the `git push origin --delete BRANCH_NAME` line; on any run of this version it is deleted, with nothing left in its place. |
...
`format` says what kind of file it is, `merge` says what agentify put into it and how, and every
...
2. **When git cannot: how was the file changed?** **Read the answer off the artifact's `format` and
   `merge` fields. Never off its path.** A file agentify *created* and did not merge into is removed
   with `rm`. A text file agentify *appended a marked block to* (`merge` absent) is un-appended with
   the marker script. Anything carrying a `merge` record goes to the un-merge script, which picks its
   arm from `merge.strategy`: `json-entries` for a config agentify merged into a data structure
   ...
   agentify appended to `.codex/config.toml`. `restore: span` names the git half of the problem; it
...
**The routing is `merge` first, then `action`** — a config agentify merged into is never removed by
`rm`, whether agentify created it or found it.

| How agentify changed the file | `manifest` fields | `manifest` list | Removed by |
|---|---|---|---|
...
| `created`, **and** merged into afterwards (agentify wrote the file *and* the entries in it) | `merge.strategy` set | `uncommitted_paths` | the **un-merge script**, which deletes the file when nothing but agentify's entries remain and keeps it when the user has since added their own; **never** a blind `rm -f` |
...
that takes agentify's own contribution back out. **Which script, and which arm of it, is decided by
...
- **A JSON config handed to the marker script.** Measured on the axios dogfood run: the marker script
  was handed `.claude/settings.json`, found no `agentify:begin` in a file that can never contain one,
  printed `SKIP .claude/settings.json: no agentify block id=…`, **exited 0**, and left the merged hook
...
- **A Codex config in no list at all.** The un-merge script's own rule used to name
  ...
  pre-existing `.codex/hooks.json` held the user's `audit-shell.sh` and `notify.sh` with agentify's
  `enforce-bun.sh` merged into the user's own `PreToolUse` group: the marker script printed
  `SKIP  .codex/hooks.json: no agentify block id=enforce-bun`, exit 0, the file byte-identical, and
...
- **A TOML config handed to the marker script — the one that does damage rather than nothing.** TOML
  *does* have `#` comments, so the marker script finds the block and cuts it, prints a reassuring
  `WARNING: … only the agentify block was removed`, and exits 0. On a fixture where the user had
  appended `approval_mode = "auto"` below agentify's block, cutting the block re-bound that key from
...
git show BRANCH_NAME:PLAN_DIR/report.md > ~/agentify-removal-steps.md
...
# rm ~/agentify-removal-steps.md    # when you no longer want the record
```
...
**Never `rm` a file the user wrote:** agentify only added to it. That case has no other exit —
...
writing paths the user deliberately excluded into a branch they may push or merge — and agentify
...
cp PLAN_DIR/report.md ~/agentify-removal-steps.md
...
what takes agentify's contribution back out. This subsection is the mechanism for a **marked text
file agentify appended to and did not merge into** — `merge` absent on its manifest entry. The next
...
# (path, agentify-id, sha256 of the file as it was before this run)
BLOCKS = [
    ("MODIFIED_PATH", "MODIFIED_ID", "PRE_EXISTING_SHA256"),
]

LEAD = r"^[ \t]*(?:<!--|//|/\*|\#|--|;|%|\*)?[ \t]*agentify[ :_-]{0,3}"
...
for path, aid, want in BLOCKS:
    ...
    if not m:
        print("SKIP  %s: no agentify block id=%s (already removed by hand?)" % (path, aid)); continue
    ...
    if sha(loose) == want:   out, note = loose, "restored byte-for-byte"
                                                 ...
                                                 "agentify block was removed; the rest of the file "
...
- **It never guesses.** If the file changed after the run, it removes the agentify block, leaves
...
**Some files agentify changes by *merging into a structure* rather than by appending a block a
...
|---|---|---|
| `json` / `json-entries` | `.claude/settings.json`, `.mcp.json`, `.codex/hooks.json` | JSON has no comment syntax, so there is nothing to mark: handed one, the marker script prints `SKIP <path>: no agentify block id=…`, exits 0, and leaves the merged hook registered forever — the measured axios defect, reproduced on `.codex/hooks.json` |
...
`templates/settings-hooks.json.tmpl` step 6 forbids merging its `_agentify` key into
...
of those the **command path** is the only mark agentify leaves, and the key is the hook artifact's
manifest `command`. `templates/mcp.json.tmpl` does write `_agentify`, and its servers are keyed by
**name** under `mcpServers`. The TOML arm keys on the `agentify:begin` / `agentify:end` comment pair
plus the recorded `[table]` names: TOML can carry a marker, so agentify writes one, and the arm still
refuses to cut a block that is no longer only agentify's.

```bash
python3 - <<'PY'
import hashlib, io, json, os, re

# (path, agentify-id, format, action, sha256 of the file as it was before this run
...
def unmerge_json(path, aid, action, want, entries, raw):
    ...
    if action == "created":
        # adapters/codex.md 4.3: agentify sets `description` only in a hooks.json it CREATED, so
...
def unmerge_toml(path, aid, action, want, entries, raw):
    ...
    re-serialises TOML: it removes a byte range agentify wrote and can still prove it wrote."""
    ...
    if '"""' in raw or "'''" in raw:
        return ("stop", 0, "", False,
                ...
                "the agentify markers by hand" % path)
    tag = r"\b[^\n]*(?<![\w-])" + re.escape(aid) + r"(?![\w-])[^\n]*"
    beg = re.compile(r"^[ \t]*#[ \t]*agentify[ :_-]{0,3}(?:begin|start)" + tag + r"\n", re.M).search(raw)
    end = re.compile(r"^[ \t]*#[ \t]*agentify[ :_-]{0,3}end" + tag + r"(?:\n|\Z)", re.M).search(raw, beg.end()) if beg else None
    ...
    if strays:
        return ("stop", 0, "", False,
                "STOP  %s: the marked block now holds a table agentify did not write (%s) -- "
    ...
    for line in raw[end.end():].splitlines():     # the TOML-only guard: a [table] header ends
        ...
        if not s.startswith("["):
            return ("stop", 0, "", False,
                    "STOP  %s: the first key after agentify's block (%s) is not under a [table] "
...
for path, aid, fmt, action, want, entries in UNMERGE:
    if outside_repo(path):
        print("STOP  %s: outside this repo -- agentify never merges into a file outside it, so "
...
- **It dispatches on the manifest, not on the path.** `format` picks the arm; `merge.entries` fills
  ...
  in this project have been the same shape: agentify wrote a file one way and un-wrote it another,
...
- **It never touches a file outside the repo.** The first guard refuses any entry that does not
  ...
  holds live credentials, and is never written by agentify in the first place (`adapters/codex.md`
...
- **The TOML arm refuses to re-bind a key it did not write.** A `[table]` header ends the table above
  it, so a bare `key = value` line *below* agentify's block belongs to agentify's last table — and
...
- **`action: created` is deleted only when nothing else is left.** A settings file agentify created
  and the user has since added `"model"` or `"permissions"` to keeps those keys and stays on disk,
  with the removal counted; so does a `config.toml` agentify created and the user has since added a
  ...
  `hooks.json` the `description` key is agentify's own — `adapters/codex.md` §4.3 lets agentify set it
...
- **A file it cannot read safely stops rather than repairs.** JSON that does not parse, a top level
  ...
  since grown a table agentify did not write: nothing is written, the user is told exactly what to do
...
agentify's `enforce-bun` merged into that same `Bash` group → `1 entry(s) removed, restored
byte-for-byte`, byte-identical to pristine, the user's `audit-bash.sh`, `user-format-guard.sh` and
`notify.sh` all still there; agentify merged as a **new** matcher group under a new event → group and
event array both pruned, byte-identical; agentify **created** the file → `deleted (agentify created it
...
agentify's `linear` and `_agentify` → `2 entry(s) removed, restored byte-for-byte`, `playwright`
...
agentify's `enforce-bun.sh` merged into the user's own group, **and** a pre-existing
`.codex/config.toml` holding their `model`, `[mcp_servers.playwright]` and
`[mcp_servers.playwright.tools.browser_navigate]`, with agentify's marked `[mcp_servers.linear]` block
...
two files with `action: created` → both `deleted (agentify created it and it now holds nothing else)`,
...
agentify's. Run twice → `FAIL … the undo did NOT complete`. A bare `approval_mode = "auto"` appended
...
PRD §3 goal 2 promises `reversible` and PRD §13 promises an uninstall path in every report — the
...
- **The marker script handed a JSON config.** `SKIP .claude/settings.json: no agentify block id=…`,
  exit 0, nothing removed — the hook agentify registered in the user's settings stays registered
...
- **The marker script handed a TOML config.** It *works*, which is why it is the worst of them:
  TOML has `#` comments, so the block is found and cut, `WARNING: … only the agentify block was
...
1. **Credentials.** Every MCP draft: the file, the variable names, the auth steps (PRD §7.9).
...
3. **An engineering system the user asked for and does not have yet** — `interview.md` Q14 answered
   `c`. agentify never installs a marketplace plugin, so this is a step with the exact install
...
server in a repo where nothing agentify built is loaded yet.

**Every item states its reason.** A numbered instruction with no reason reads as a tool that gave
up; the reason is what makes it read as a boundary agentify chose not to cross. The four recurring
...
| a new session (both targets) | agentify runs inside your current session and cannot restart it; the harness fixed its hook and config set when that session started |
...
| MCP auth (both targets) | the key must never be written into a repo file, pasted into this conversation, or held by agentify |
...
4. Without it, `.codex/hooks.json`, `.codex/rules/agentify.rules` and `.codex/config.toml` do
