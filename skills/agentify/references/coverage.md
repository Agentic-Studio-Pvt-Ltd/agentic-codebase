...
ranked candidate list. This file replaces the sizing-cap table that earlier versions of agentify
applied here.
...
**agentify does not limit how many artifacts it builds, and never tells the user a limit stopped
...
must say what was missing rather than implying agentify held back.
...
itself. Re-run agentify after a week of normal usage and the setup will also be shaped by what you
actually ask for.
```
...
- **A rerun** updates artifacts carrying an `agentify-id` in place and never duplicates them. An
