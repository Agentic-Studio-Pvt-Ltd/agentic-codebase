...
| Q13 | **I found N low-confidence findings I'd normally skip. Want them included?** | `b) no — skip them` | `low`. At least 2 phase-3 findings are confidence `low` AND the caps have room. | Moves those candidates from "skipped" into the build list. Low-confidence findings are never built without this explicit yes. |
...
|---|---|
| Skill, subagent | `blueprint.md` §6.1 / §3, plus the three conditions in `mapping-rules.md` §3 |
