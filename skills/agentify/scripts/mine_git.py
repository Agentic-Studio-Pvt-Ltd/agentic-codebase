...
PARTIAL (BLOBLESS) CLONES -- DEGRADE CHURN, KEEP EVERYTHING ELSE
    `test-repos.md` tells contributors to clone the regression fixtures with
...
SAFETY -- READ-ONLY, ENFORCED
    ...
    contract (PRD 13) is absolute: this tool never writes to the user's git
...
THE ONE OPTIONAL NETWORK PATH
    ...
    github.com.  It is the ONLY code path in the whole of agentify that can
...
#: `--filter=blob:none` clone (what `test-repos.md` tells contributors to make)
...
def _gh(
    ...
    in agentify; see the module docstring.
...
def selftest() -> int:
    ...
    degraded_ok = True
    degraded_detail = []
    for label, path in (("missing", os.path.join(tempfile.gettempdir(), "agentify-no-such-repo")),
