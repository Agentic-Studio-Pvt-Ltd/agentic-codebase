...
# -*- coding: utf-8 -*-
"""
agentify :: scripts/mine_transcripts.py -- the transcript miner.
...
What this script will and will not do (PRD 13, contract "Non-negotiables"):
...
Five shaping rules the consumers depend on:
  ...
  * SWEARING IS MASKED, THE DIRECTIVE IS KEPT.  Every emitted quote passes
    ...
    `report.md` and an `agentify-evidence:` frontmatter line.  Masking runs
...
#
#   /Users/x/Documents/projects/hostshare/ravisojitra
#     -> -Users-x-Documents-projects-hostshare-ravisojitra
...
def _score_candidate(encoded, name):
    ...
    0.80  the name is the repo plus more segments (a cwd *inside* the repo)
          ...
          that guard `.../projects/hostshare/ravisojitra` scores 0.67 against
          `.../projects/riffads`, which is a different project entirely.
    """
    if name == encoded:
        return (1.0, True)
...
#: most common way a developer states a convention -- it is the PRD's own
...
#: evidence string in `plan.md`, in `report.md` and in the `agentify-evidence:`
...
#: agentify removed it.  Rewriting the sentence into clean prose would change
...
def selftest():
    ...
    add(
        ...
        encode_project_dir("/Users/x/Documents/projects/hostshare/ravisojitra")
        == "-Users-x-Documents-projects-hostshare-ravisojitra"
    ...
    sandbox = tempfile.mkdtemp(prefix="agentify-mt-selftest-")
