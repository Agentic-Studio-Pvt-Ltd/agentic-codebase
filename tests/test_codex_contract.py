...
def render_hook(event="PreToolUse", check="return 1", file_scoped=False):
    ...
    values = dict(
        AGENTIFY_ID="contract-test", GENERATED_DATE="2026-09-16",
...
class CodexHookContract(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="agentify-contract-")
    ...
    def test_shell_wiring_and_canonical_name(self):
        ...
        fragment = text.split("<<<AGENTIFY-FRAGMENT-BEGIN>>>")[1].split(
            "<<<AGENTIFY-FRAGMENT-END>>>")[0].strip()
